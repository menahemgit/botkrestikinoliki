...
def get_inline_menu_button():
    keyboard = [[InlineKeyboardButton("Открыть меню", callback_data='open_menu')]]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(welcome_message, reply_markup=get_inline_menu_button())
    return MENU
...
async def open_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(text=menu_message, reply_markup=get_main_keyboard())
    return MENU
...
async def send_daily_stats(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Например:
    ADMIN_CHAT_ID = 'ваш_id_чата_администратора'  # Замените на реальный ID
    await context.bot.send_message(chat_id=ADMIN_CHAT_ID, text=stats_message)

async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    logger.error(f"Exception while handling an update: {context.error}")
    # Здесь вы можете добавить отправку сообщения об ошибке администратору
    # Например:
    ADMIN_CHAT_ID = 'ваш_id_чата_администратора'  # Замените на реальный ID
    await context.bot.send_message(
...
# Функции для клавиатур и обработки сообщений (оставьте их без изменений)
