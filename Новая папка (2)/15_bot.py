...
async def play_with_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Игра с ботом начата!")
    await send_board(game_code, context)
    return GAME_PLAY
...
async def end_bot_game(game_code, result, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(game['player1'], "Выберите действие:", reply_markup=get_main_keyboard())
    del active_games[game_code]
...
# Функция для проверки и обновления ежедневного задания
...
# Функция для отправки уведомлений неактивным игрокам
async def send_reminder(context: ContextTypes.DEFAULT_TYPE) -> None:
    current_time = time.time()
    for player_id, stats in player_stats.items():
        if 'last_activity' in stats and current_time - stats['last_activity'] > 86400 * 3:  # 3 дня неактивности
            await context.bot.send_message(
                player_id,
                "👋 Привет! Давно не виделись. Может, сыграем партию в крестики-нолики?"
            )
...
async def main() -> None:
    ...
    job_queue.run_repeating(auto_save, interval=300, first=10)  # Автосохранение каждые 5 минут
    job_queue.run_daily(cleanup_games, time=datetime_time(hour=3, minute=0))  # Очистка в 3:00 каждый день
    job_queue.run_daily(send_reminder, time=datetime_time(hour=18, minute=0))  # Отправка напоминаний в 18:00 каждый день
