...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    stats = player_stats.get(user_id, {"wins": 0, "losses": 0, "draws": 0})
...
async def end_game(game_code, result, context):
    ...
    for player in [game['player1'], game['player2']]:
        if player != 'bot':
            try:
                await context.bot.send_message(chat_id=player, text=message, reply_markup=reply_markup)
            except Exception as e:
...
async def load_data():
    global player_stats, active_games
    try:
...
async def save_data():
    try:
        with open('game_data.json', 'w') as f:
            json.dump({'player_stats': player_stats, 'active_games': active_games}, f)
...
async def main() -> None:
    ...
    # Обработчики для кнопок и игрового процесса
    application.add_handler(CallbackQueryHandler(post_game_button_handler, pattern="^(play_again|main_menu)$"))
    application.add_handler(CallbackQueryHandler(main_menu_button_handler, pattern="^(new_game|stats|rules)$"))
